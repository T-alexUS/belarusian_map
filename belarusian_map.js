...
w.data.rows = w.data.rows.map(item=>[item[0].toUpperCase()]);
...
var tooltip = {
    ...
    pointFormatter: function () {
        ...
        return (this['short-name'] ? this['short-name'] : this.name) + ': ' + formattedValue + '<br/>'
    },
    style: {
...
function DisplayMap(mD) {
    ...
    series[0].point = {
        events: {
            click: function () {
                ...
                if (this.key == 'ru') {
                    DisplayMap(Highcharts.maps['russia-wiki']);
    ...
    mapChart = Highcharts.mapChart(w.general.renderTo, {
        chart: {
            ...
            marginBottom: w.props.marginBottom ? w.props.marginBottom : 0,
            marginTop: w.props.marginTop ? w.props.marginTop : 0,
        ...
        plotOptions: {
            map: {
                ...
                states: {
                    select: {
                        color: (w.props.selectedRegionColor == 'hatch') ? 'url(#highcharts-default-pattern-1)' : w.props.selectedRegionColor
                    }
                }
            },
            series: {
                nullColor: w.props.nullColor? w.props.nullColor: "#B5B5B5",
...
function prepareSeries(w, mD) {
    ...
    var _data = _.zip(w.data.rows, w.data.values[0], w.data.values[1]).map(function (item, i) {
